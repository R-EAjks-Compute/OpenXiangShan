...
class DataPath(implicit p: Parameters, params: BackendParams, param: SchdBlockParams)
  ...
  private val intRfBankRaddrWidth = log2Ceil(intPregNumBank)
  println(s"intPregNumBank = $intPregNumBank")
  println(s"intRfBankRaddrWidth = $intRfBankRaddrWidth")
  ...
  if (param.isIntSchd) {
    println(s"intPregNumBank = $intPregNumBank")
    println(s"intRfBankRaddrWidth = $intRfBankRaddrWidth")
    println(s"intSchdParams.pregIdxWidth = ${intSchdParams.pregIdxWidth}")
    val intRfRaddr = Wire(Vec(params.numPregRd(IntData()), Vec(intPregNumBank, UInt((intSchdParams.pregIdxWidth - intRfBankRaddrWidth).W))))
