...
class Ghr(implicit p: Parameters) extends GhrModule with Helpers {
  ...
  private val ghrStall = Module(new Queue(new GhrEntry, StallQueueSize, pipe = true, flow = true, hasFlush = true))
  ...
  ghrStall.io.enq.valid := s3_fire
  ghrStall.io.enq.bits  := s3_ghr
  ghrStall.io.flush.get := r0_valid
  ghrStall.io.deq.ready := s0_fire
