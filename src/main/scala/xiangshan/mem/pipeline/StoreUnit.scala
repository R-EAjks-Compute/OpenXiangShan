...
class StoreUnit(val param: ExeUnitParams)(implicit p: Parameters) extends XSModule
  ...
  val s0_misalignNeedReplay = (s0_use_flow_vec || s0_rs_corss16Bytes) && !(s0_uop.sqIdx === io.sqCommitPtr || s0_uop.robIdx === io.sqCommitRobIdx && s0_uop.vpu.vuopIdx === io.sqCommitUopIdx)
